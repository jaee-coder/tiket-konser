...
<div class="max-w-2xl mx-auto bg-white rounded-xl shadow p-6">
    <h1 class="text-2xl font-bold mb-6">Edit Konser</h1>

    <form method="POST" action="{{ route('admin.konsers.update', $konser->id_concert) }}">
        @csrf
        @method('PUT')

        <div class="mb-4">
            <label for="nama_concert" class="block text-gray-700 font-medium mb-1">Nama Konser <span class="text-red-500">*</span></label>
            <input type="text" name="nama_concert" id="nama_concert" value="{{ old('nama_concert', $konser->nama_concert) }}" required
        ...
        <div class="mb-4">
            ...
            <input type="time" name="waktu" id="waktu" value="{{ old('waktu', $konser->waktu) }}" required
                   class="w-full border rounded-lg px-3 py-2">
        ...
        <div class="mb-4">
            ...
            <input type="number" name="harga_dasar" id="harga_dasar" value="{{ old('harga_dasar', $konser->harga_dasar) }}" required min="0" step="1000"
                   class="w-full border rounded-lg px-3 py-2">
        ...
        <div class="mb-4">
            ...
            <select name="id_venue" id="id_venue" required class="w-full border rounded-lg px-3 py-2">
                ...
                @foreach($venues as $venue)
                    <option value="{{ $venue->id_venue }}" {{ old('id_venue', $konser->id_venue) == $venue->id_venue ? 'selected' : '' }}>
                        {{ $venue->nama_venue }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="flex justify-end space-x-2">
            ...
            <button type="submit" class="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg shadow transition">Update</button>
        </div>
    </form>
</div>
@endsection